...
import { deleteMemo as deleteMemoAction } from "@/store/reducer/memo";
...
const Memo: React.FC<Props> = (props: Props) => {
  ...
  const handleArchiveMemoClick = async () => {
    try {
      ...
      // 归档成功后从列表中移除（只从 store 中删除，不调用 DELETE API）
      dispatch(deleteMemoAction(memo.id));
      toast.success(t("memo.archived-successfully"));
    } catch (error: any) {
      console.error(error);
      toast.error(error.response?.data?.message || "Failed to archive memo");
    }
  };
